<%@ page language="java" contentType="text/html; charset=UTF-8"
    pageEncoding="UTF-8"%>
<!DOCTYPE html>
<html>
<head>
<meta charset="UTF-8">
<meta name="viewport" content="width=device-width, initial-scale=1">
<link rel="stylesheet" href="resources/css/include.css">
<title>Insert title here</title>
</head>
<body>

	<div class="layout_container">
		
		<div class="footer_desktop footer">
			
			<div class="column_left">
				<p class="company_info1">상호: (주)페어테이블 | 대표: 박성호 | 개인정보관리책임자: 성동현 | 전화: 070-4145-5928 | 이메일: FAIRTABLE@DAUM.NET</p>
				<p class="company_info2">주소: 서울특별시 송파구 법원로 11길 25(문정동) 에이치비즈니스파크 A동 205호 | 사업자등록번호: 635-87-00618 | 통신판매: 제 2017-서울송파-1128 호 | 호스팅제공자: (주)식스샵</p>
				<p class="company_info3">Copyright ⓒ 페어테이블. All rights reserved</p>
			</div>
			
			<div class="column_right">
				<div class="sns_group">
					<div class="icon">[전화]</div>
					<div class="icon">[메세지]</div>
					<div class="icon">[인스타그램]</div>
				</div>
			</div>
			
		</div>
	</div>
	
</body>
</html>