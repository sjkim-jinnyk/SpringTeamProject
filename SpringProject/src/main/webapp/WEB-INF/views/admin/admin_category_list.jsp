				...
				<div class="row">
					<div class="col-md-12">
						<div class="admin-category">
									...
									<c:forEach items="${List }" var="dto" varStatus="status">
										<tr class="pro-wrap">
											...
											<td class="cate-name">
												...
												<div id="cate-update-${status.index }"
													style="display: none;">
													<input type="text" id="cate-name-${status.index }"
														name="cate_name" value="${dto.getCate_name() }">
												</div>
											</td>
											<td class="cate-btn">
												<div id="btn-${status.index }" class="pro-btn-wrap">
													<button class="update-btn"
														onclick="makeCate(${status.index });">수정</button>
													<button class="delete-btn"
														onclick="deleteCate(${dto.getCate_no() },'${dto.getCate_name() }');">삭제</button>
												</div>
												<div id="btn-update-${status.index }" class="pro-btn-wrap"
													style="display: none;">
													<button class="delete-btn"
														onclick="updateCate(${status.index});">등록</button>
													<button class="default-btn"
														onclick="makeCate(${status.index });">취소</button>
												</div>
											</td>
										</tr>
							...
							<span class="info">* 카테고리 삭제 시 기존에 등록되어 있던 상품의 카테고리는 공란
								처리됩니다.</span>
						</div>
					</div>
				</div>
