...
threeMonth.addEventListener('click', function() {
	...
	let today = getToday();
	document.reviewForm.orderFirst.value = three;
	document.reviewForm.orderLast.value = today;
...
oneMonth.addEventListener('click', function() {
	let one = getMonth();
	let today = getToday();
	document.reviewForm.orderFirst.value = one;
	document.reviewForm.orderLast.value = today;
...
sevenDay.addEventListener('click', function() {
	let seven = getSeven();
	let today = getToday();
	document.reviewForm.orderFirst.value = seven;
	document.reviewForm.orderLast.value = today;
...
allDay.addEventListener('click', function() {
	...
	$.ajax({
		...
		success : function(data) {
			document.reviewForm.orderFirst.value = data;
			document.reviewForm.orderLast.value = today;
		},
		error : function() {
			alert('오류');
...
function showPopup(no) {
	...
	window.open("./member_review_write.do?no="+no, "후기작성", "width=530, height=530, left=100, top=50");
}