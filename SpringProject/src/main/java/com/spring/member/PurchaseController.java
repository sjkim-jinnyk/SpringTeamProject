...
public class PurchaseController {
	...
	public String purchase(HttpSession session, Model model) throws IOException {
		...
		if (clist.size() == 1) {
			...
			orderCont = productDTO.getPro_name() + " " + clist.get(0).getCart_amount() + "개 외 " + (clist.size() - 1)
					+ "종";
			System.out.println("주문내역 : " + orderCont);
		}
	...
	public String order(HttpSession session, HttpServletRequest request) {
		...
		if (temp2 != "") {
			...
			coupon_type = Integer.parseInt(request.getParameter("coupon_type"));
		}
		...
		for (CartDTO cartDTO : clist) {
			...
			ProductDTO productDTO = this.productDAO.getProductCont(cartDTO.getProduct_no());
			String cont = productDTO.getPro_name() + " " + cartDTO.getCart_amount() + "개"; // 주문내역 성형
			orderDTO.setOrder_content(cont);
			orderDTO.setCoupon_no(coupon_no);

			int price = productDTO.getPro_output_price() * cartDTO.getCart_amount() * (100 - coupon_type) / 100; // 각각의 가격 계산
